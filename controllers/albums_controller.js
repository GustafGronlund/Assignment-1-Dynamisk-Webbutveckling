...
const getAlbums = async (req, res) => {
  try {
    Users.where({
      ...
      .then(function (data) {
        data = data.toJSON();
        let newData = data.findIndex((item) => {
          return item.field === "albums";
        });
        res.status(200).send({
          status: "success",
          data: data[0].albums,
...
const uploadAlbum = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      status: "error",
      message:
        "Something went wrong when trying to add a new album, please try again.",
    });
    throw error;
  }
};
...
const updateSingleAlbum = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({
      status: "error",
      message:
        "Something went wrong when trying to update the selected album, please try again.",
    });
    throw error;
  }
};
...
const addPhotoToAlbum = async (req, res) => {
  ...
  anvandare_photo = users_photos.find((photo) => photo.id == req.body.photo_id);
  anvandare_album = users_albums.find((album) => album.id == req.params.id);

  // hämtar albumet med det id som är skrivet i url, plockar med de relaterade fotografierna
  const album = await new models.Albums({ id: req.params.id }).fetch({
    withRelated: ["photos"],
  });
  ...
  try {
    ...
    res.status(500).send({
      status: "error",
      message:
        "Something went wrong when trying to add the your photo to the selected album.",
    });
    throw error;
  }
};
...
const getSingleAlbum = async (req, res) => {
  ...
  existing_album = related_albums.find((album) => album.id == req.params.id);
  ...
  const chosen_album = await new models.Albums({
    id: req.params.id,
  }).fetch({
    withRelated: ["photos"],
  });
